...
OPENSSL_FLAG=-DOPENSSL_INCLUDE_DIR=/usr/include/openssl-1.1 -DOPENSSL_SSL_LIBRARY=/usr/lib/libssl.so.1.1 -DOPENSSL_CRYPTO_LIBRARY=/usr/lib/libcrypto.so.1.1

BUILD_FLAGS=-DEXTENSION_STATIC_BUILD=1 $(EXTENSION_FLAGS) ${EXTRA_EXTENSIONS_FLAG} $(OSX_BUILD_FLAG) $(TOOLCHAIN_FLAGS) -DDUCKDB_EXPLICIT_PLATFORM='${DUCKDB_PLATFORM}' $(OPENSSL_FLAG) -DBUILD_PYTHON=1
